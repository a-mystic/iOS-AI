...
extension UIImage {
    ...
    func rotateBy(degrees: CGFloat, clockwise: Bool = false) -> UIImage? {
        ...
        let centreRect = CGRect(x: -self.size.width/2, y: -self.size.height/2, width: self.size.width, height: self.size.height)
...
extension Collection where Element == VNFaceObservation {
    func drawOn(_ image: UIImage) -> UIImage? {
        ...
        let imageSize: (width: Int, height: Int) = (Int(width), Int(height))
