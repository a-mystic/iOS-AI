...
class EmotionVisualizerView: UIView {
    ...
    var valueFontColor : UIColor = UIColor.white{
        didSet{
            if self.animTimer == nil{
                self.setNeedsDisplay()
            }
...
extension EmotionVisualizerView{
    ...
    override public func draw(_ rect: CGRect) {
        ...
        let labelAttrs = [NSAttributedString.Key.font: UIFont(name: "HelveticaNeue", size: self.labelFontSize)!,
                          NSAttributedString.Key.paragraphStyle: labelParagraphStyle,
                          NSAttributedString.Key.foregroundColor : self.labelFontColor]
        ...
        for (k, v) in predictions{
            ...
            k.draw(
                with:CGRect(x: -(y + h - 10), y: x+w/4, width: 100, height: w),
